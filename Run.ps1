...
function Invoke-AppInstallsSubmenu {
    ...
    while ($true) {
        ...
        if ($rawTrim -match '^[Aa]$') {
            $toRun = $categories
        } elseif ($rawTrim -match '^[0-9]+$') {
            $n = [int]$rawTrim
            if ($n -ge 1 -and $n -le $categories.Count) { $toRun = @($categories[$n - 1]) }
        } else {
            $nums = @()
            foreach ($tok in ($rawTrim -split '[,;\s]+')) {
                if ($tok -match '^\d+$') {
                    $val = [int]$tok
                    if ($val -ge 1 -and $val -le $categories.Count -and -not $nums.Contains($val)) { $nums += $val }
                }
            }
            if ($nums -and $nums.Count -gt 0) {
                foreach ($n in $nums) { $toRun += $categories[$n - 1] }
            }
        }
        ...
        foreach ($cat in $toRun) {
            ...
            $sel = Select-AppsFromManifest -Category $cat -Apps $manifest
            if ($sel -and $sel.Count -gt 0) { $allSelected.AddRange($sel) }
        }
        if ($allSelected.Count -gt 0) {
            $label = if ($toRun.Count -gt 1) { ($toRun -join ',') } else { $toRun[0] }
            Start-WingetInstallBatch -SelectedApps $allSelected.ToArray() -Category $label
        }
...
function Select-AppsFromManifest {
    param(
        [Parameter(Mandatory = $true)] [string] $Category,
...
# Create a temporary script with winget install commands and run it elevated in a new process
function Start-WingetInstallBatch {
    param(
        [Parameter(Mandatory = $true)] [object[]] $SelectedApps,
        [Parameter(Mandatory = $true)] [string] $Category
    )
    try {
        $tempPath = Join-Path $env:TEMP ("winutil-winget-" + [System.Guid]::NewGuid().ToString('N') + '.ps1')
        $lines = New-Object System.Collections.Generic.List[string]
        [void]$lines.Add("# Auto-generated by winutil for category '$Category'")
        [void]$lines.Add("winget source update --disable-interactivity --silent")
        foreach ($app in $SelectedApps) {
            $id = $app.Id
            $name = $app.Name
            [void]$lines.Add([string]::Format("Write-Host 'Installing: {0} ({1})' -ForegroundColor Cyan", $name, $id))
            [void]$lines.Add([string]::Format('winget install --id "{0}" --exact --source winget --accept-source-agreements --accept-package-agreements --disable-interactivity --silent', $id))
        }
        Set-Content -Path $tempPath -Value ($lines -join [Environment]::NewLine) -Encoding UTF8 -Force

        $psExe = Get-PreferredPowerShellExe
        Start-Process -FilePath $psExe -ArgumentList @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File', $tempPath) -Verb RunAs | Out-Null
        Write-Host ("Opened winget installer for '{0}' in a new elevated window." -f $Category) -ForegroundColor Green
    } catch {
        Write-Host ("Failed to start winget batch for '{0}': {1}" -f $Category, $_.Exception.Message) -ForegroundColor Red
    }
}
